...
document.getElementById("searchInput").addEventListener("input", function(event) {
    const searchTerm = event.target.value.trim().toLowerCase(); // Get the entered search term
    
    // Select all song items
    const songItems = document.querySelectorAll(".songList ul li");

    // Loop through each song item to check if it matches the search term
    songItems.forEach(function(songItem) {
        const songName = songItem.textContent.toLowerCase(); // Get the song name
        
        // If the song name contains the search term, display the song item, otherwise hide it
        if (songName.includes(searchTerm)) {
            songItem.style.backgrounColor = "red";
        } else {
